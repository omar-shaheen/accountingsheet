...
$.fn.SetEditable = function (options) {
   ...
   this.find('thead tr'); //encabezado vacío
...
function rowAcep(but) {
   ...
   var $row = $(but).parents('tr'); //accede a la fila
   var $cols = $row.find('td.editthis'); //lee campos
...
function rowAddNew(tabId) { //Agrega fila a la tabla indicada.
   ...
   if ($filas.length == 0) {
      ...
      var $cols = $ultFila.find('td.editthis'); //lee campos
      $cols.each(function () {
         if ($(this).attr('name') == 'buttons') {
            //Es columna de botones
         } else {
...
function rowAddNewCol(tabId) { //Agrega fila a la tabla indicada.
   ...
   if ($filas.length == 0) {
      ...
      $filas.each(function () {
         ...
         //$(this).append('<td></td>');
         $(this).find('td.editthis:last').before('<td></td>');
      });
   }
...
function TableToCSV(tabId, separator) { //Convierte tabla a CSV
   ...
   $tab_en_edic.find('tbody tr').each(function () {
      ...
      var $cols = $(this).find('td.editthis'); //lee campos
      datFil = '';
      $cols.each(function () {
         if ($(this).attr('name') == 'buttons') {
            //Es columna de botones
...
$("#dyanmictable2").SetEditable({
   $addButton: $('#addNewRow2'),
   onEdit: function () {
      // console.log(this);
   }
});
